...
<?php 
   ...
   $programId = $selExamRow['prog_id'];
   $selProgram = $conn->query("SELECT prog_name as programName FROM program_tbl WHERE prog_id='$programId' ")->fetch(PDO::FETCH_ASSOC);
 ?>
                           ...
                           <form method="post" id="updateExamFrm">
                            <!-- <div class="form-group">
                                ...
                                <select class="form-control"  required disabled>
                                  <option value="<?php echo $selExamRow['prog_id']; ?>"><?php echo $selProgram['programName']; ?></option>
                                </select>
                              </div> -->
                              <div class="form-group">
                                <label>Course</label>
                                <select class="form-control" name="courseId" required disabled>
                              ...
                              <div class="form-group">
                                ...
                                <input type="text" name="examTitle" class="form-control" required value="<?php echo $selExamRow['ex_title']; ?>">
                              </div>  

                              <div class="form-group">
                                <label>Exam Description</label>
                                <input type="text" name="examDesc" class="form-control" required="" value="<?php echo $selExamRow['ex_description']; ?>">
                              </div>  
                          ...
                          <div class="card-header"><i class="header-icon lnr-license icon-gradient bg-plum-plate"> </i>Exam Question's 
                            <span class="badge badge-pill badge-primary ml-2">
                             ...
                             <div class="btn-actions-pane-right">
                                <button class="btn btn-sm btn-primary " data-toggle="modal" data-target="#modalForAddQuestionEx">Add Question</button>
                              </div>
                          </div>
<div class="card-body" >
...
<table class="align-middle mb-0 table table-borderless table-striped table-hover" id="tableList">
  <thead>
  <tr>
    <th class="text-left pl-1">Question Name</th>
    ...
    <tbody>
     ...
     if($selQuest->rowCount() > 0)
      {
        $i = 1;
        while ($selQuestionRow1 = $selQuest->fetch(PDO::FETCH_ASSOC)) {
          
          $qid=$selQuestionRow1['que_id'];
          $sql1 = $conn->query("SELECT * FROM question_tbl WHERE eqt_id='$qid'");
          $selQuestionRow = $sql1->fetch(PDO::FETCH_ASSOC)
         ?>
<tr>
  <td>
    <b>
      <?php echo $i++ ; ?> .) <?php echo $selQuestionRow['exam_question']; ?></b><br>
   <?php
    // Choice A
    ...
    if($selQuestionRow['exam_ch4'] == $selQuestionRow['exam_answer'])
     { ?>
      ...
      <span class="pl-4">D - <?php echo $selQuestionRow['exam_ch4']; ?></span><br><?php } ?>
     
      </td>
      <td class="text-center">
        <button type="button" id="removeQuestion" data-id='<?php echo $selQuestionRow['eqt_id']; ?>'  class="btn btn-warning btn-sm">Remove</button>
      </td>
        </tr>
   <?php }
 }
