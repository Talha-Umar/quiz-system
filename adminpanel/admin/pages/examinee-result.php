                                ...
                                if($selExmne->rowCount() > 0)
                                {
                                    while ($selExmneRow = $selExmne->fetch(PDO::FETCH_ASSOC)) { ?>
                                        <tr>
                                           ...
                                           <td>
<?php
$selScore = $conn->query("SELECT * FROM exam_question_tbl ext INNER JOIN question_tbl eqt INNER JOIN exam_answers ea ON eqt.eqt_id = ea.quest_id WHERE  ea.quest_id=ext.que_id   AND ea.exans_status='new' "); 
?>
<span>
 <?php echo $selScore->rowCount(); ?>
    <?php $over  = $selExName['ex_questlimit_display'];?>
     </span> / <?php echo $over; ?>
  </td>
 <td>
    <?php 
