...
$selExmne = $conn->query("SELECT * FROM examinee_tbl ORDER BY exmne_id DESC ");
    ...
    <tr>
        ...
        <td>
            <?php 
                 $exmneProgram = $selExmneRow['exmne_program'];
                $selProgram = $conn->query("SELECT * FROM program_tbl WHERE prog_id='$exmneProgram' ")->fetch(PDO::FETCH_ASSOC);
                 echo $selProgram['prog_name'];
            ?>
        </td>
        <td>
           <?php 
